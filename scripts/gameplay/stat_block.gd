...
static func apply_equipment(base_stats: Dictionary, _config: GameConfigResource) -> Dictionary:
	...
	for slot in effective["equipment"].keys():
		...
		var item_id: StringName = (
			item_id_value if item_id_value is StringName else StringName(str(item_id_value))
		)
		if item_id.is_empty():
			continue
		...
		var upgrade_level: int = int(
			player_item.get("current_upgrade_level", player_item.get("currentUpgradeLevel", 1))
		)
		...
		effective["max_hp"] = (
			effective.get("max_hp", effective.get("maxHp", 0))
			+ modifiers.get("max_hp", modifiers.get("maxHp", 0))
		)
		effective["max_armor"] = (
			effective.get("max_armor", effective.get("maxArmor", 0))
			+ modifiers.get("max_armor", modifiers.get("maxArmor", 0))
		)
		effective["attack"] = effective.get("attack", 0) + modifiers.get("attack", 0)

		var coin_mod: float = modifiers.get("coin_multiplier", modifiers.get("coinMultiplier", 1.0))
		if coin_mod != 1.0:
			effective["gold_multiplier"] = effective.get("gold_multiplier", 1.0) * coin_mod

		var xp_mod: float = modifiers.get("xp_multiplier", modifiers.get("xpGainMultiplier", 1.0))
		if xp_mod != 1.0:
...
static func _clamp_caps(stats: Dictionary) -> Dictionary:
	...
	stats["armor"] = clamp(
		stats.get("armor", 0), 0, stats.get("max_armor", stats.get("maxArmor", 0))
	)
	return stats
