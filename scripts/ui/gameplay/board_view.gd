...
func _layout_tiles() -> void:
	...
	_tile_size = size / board_size
	var offset := (rect_size - Vector2(size, size)) * 0.5
	_grid_origin = offset

	for pos in _tile_nodes.keys():
		var tile_view: Control = _tile_nodes[pos]
		var pixel_pos := offset + Vector2(pos.x, pos.y) * _tile_size
...
func _gui_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		...
		position = to_local(event.position)
		var cell := _point_to_cell(position)
		if event.pressed:
			_start_path(cell)
		else:
...
func _is_tile_compatible(cell: Vector2i) -> bool:
	if _current_path.is_empty():
		return true
	var start_tile := _board[_current_path[0].y][_current_path[0].x]
	var base_type := str(start_tile.get("type"))
	var current_tile := _board[cell.y][cell.x]
