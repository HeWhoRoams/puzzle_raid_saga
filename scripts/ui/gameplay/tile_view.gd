...
func _update_visuals() -> void:
	...
	var color := TYPE_COLORS.get(type_name, Color(0.2, 0.2, 0.25))
	if _is_selected:
		color = color.lightened(0.3)
	background.color = color

	if type_name == "SKULL":
		var hp := tile_data.get("hp", 0)
		var max_hp := tile_data.get("max_hp", tile_data.get("maxHp", hp))
...
func _update_intent_icon(traits: Array) -> void:
	if traits.is_empty():
		...
		return
	var trait := traits[0]
	var path := "res://art/icons/traits/%s.png" % trait.to_lower()
