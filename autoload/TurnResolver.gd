...
func resolve_player_action(
	board: Array, base_stats: Dictionary, path: Array[Vector2i]
) -> Dictionary:
	...
	match path_type:
		...
		TILE_SHIELD:
			_resolve_shield_chain(
				player_stats, effective_stats, chain_length, chain_multiplier, logs
			)
		TILE_POTION:
			_resolve_potion_chain(
				player_stats, effective_stats, chain_length, chain_multiplier, logs
			)
		TILE_COIN:
...
func resolve_enemy_pre_attacks(board: Array) -> Dictionary:
	...
	var heal_amount: int = 5 * healer_tiles.size()
	var total_healed: int = 0
	for row in new_board:
		for tile in row:
			if tile and tile.get("type") == TILE_SKULL:
				var hp: int = tile.get("hp", 0)
				var max_hp: int = tile.get("max_hp", hp)
				var delta: int = min(heal_amount, max_hp - hp)
				if delta > 0:
					tile["hp"] = hp + delta
					total_healed += delta
...
func resolve_enemy_attacks(
	board: Array, base_stats: Dictionary, effective_stats: Dictionary, path: Array[Vector2i]
) -> Dictionary:
	var new_player := StatBlock.duplicate(base_stats)
	var logs: Array[String] = []
	var total_armor_damage: int = 0
	var total_health_damage: int = 0
	var total_pierce: int = 0
	var poison_stacks: int = new_player.get("poison_stacks", 0)
	...
	for row_idx in range(board.size()):
		for col_idx in range(board[row_idx].size()):
			var tile: Dictionary = board[row_idx][col_idx]
			if tile == null:
				continue
			...
			if path_lookup.has(tile_pos):
				continue
			var attack_value: int = tile.get("attack", 0)
			if attack_value <= 0:
				continue
			...
			if "ARMOR_PIERCING" in traits:
				...
				var mitigated: int = max(1, attack_value - effective_stats.get("armor", 0))
	...
	if total_pierce > 0:
		...
		logs.append(
			"[color=violet]Armor-piercing attacks deal %s direct damage![/color]" % total_pierce
		)

	if total_health_damage > 0:
		var armor_damage: int = min(new_player.get("armor", 0), total_health_damage)
		new_player["armor"] = max(0, new_player.get("armor", 0) - armor_damage)
		var remaining: int = total_health_damage - armor_damage
		if remaining > 0:
		...
		(
			...
			. append(
				(
					"Enemies attack! [color=orange]%s[/color] damage, [color=lightblue]%s[/color] armor lost."
...
func tick_down_cooldowns_and_buffs(player_stats: Dictionary) -> Dictionary:
	...
	if new_player.has("buffs"):
		...
		for i in range(buffs.size() - 1, -1, -1):
			...
			var duration: int = buff.get(duration_key, 0) - 1
			if duration <= 0:
				buffs.remove_at(i)
...
func resolve_level_ups(player_stats: Dictionary, class_id: StringName) -> Dictionary:
	...
	var class_def := ContentDB.get_class_definition(class_id)
	if class_def == null:
...
func apply_poison_damage(player_stats: Dictionary) -> Dictionary:
	var new_player := StatBlock.duplicate(player_stats)
	var stacks: int = new_player.get("poison_stacks", 0)
	var logs: Array[String] = []
	if stacks > 0:
...
func _resolve_attack_chain(
	...
	var total_damage := int(
		floor(
			effective_stats.get("attack", 0) * chain_length * chain_multiplier * attack_multiplier
		)
	)
	if total_damage <= 0:
		return
	...
	for pos in path:
		var tile: Dictionary = board[pos.y][pos.x]
		if tile.get("type") == TILE_SKULL:
	...
	for target in skull_targets:
		...
		var hp: int = tile.get("hp", 0)
		var damage_to_tile: int = min(remaining_damage, hp)
		tile["hp"] = hp - damage_to_tile
		remaining_damage -= damage_to_tile

		if tile["hp"] <= 0:
			var enemy_def: EnemyDefinitionResource = ContentDB.get_enemy(tile.get("enemy_id", ""))
			if enemy_def:
			...
			logs.append(
				(
					"%s takes [color=red]%s[/color] damage."
					% [tile.get("name", "Enemy"), damage_to_tile]
				)
			)


func _resolve_shield_chain(
	...
	var gain: int = int(floor(5 * length * multiplier))
	player_stats["armor"] = min(
		effective_stats.get("max_armor", player_stats.get("max_armor", 0)),
		player_stats.get("armor", 0) + gain
	)
	logs.append("You repaired [color=lightblue]%s[/color] armor." % gain)


func _resolve_potion_chain(
	player_stats: Dictionary,
	effective_stats: Dictionary,
	length: int,
	multiplier: float,
	...
	var heal: int = int(floor(10 * length * multiplier))
	player_stats["hp"] = min(
		effective_stats.get("max_hp", player_stats.get("max_hp", 0)),
		player_stats.get("hp", 0) + heal
	)
	logs.append("You healed for [color=green]%s[/color] HP." % heal)


func _resolve_coin_chain(
	player_stats: Dictionary,
	effective_stats: Dictionary,
	length: int,
	multiplier: float,
	...
	var gold_multiplier: float = effective_stats.get("gold_multiplier", 1.0)
	...
	var gold_gained: int = int(floor(length * multiplier * gold_multiplier))
...
func activate_ability(ability_id: StringName, board: Array, player_stats: Dictionary) -> Dictionary:
	var new_board: Array = BoardService.duplicate_board(board)
	var new_player := StatBlock.duplicate(player_stats)
	var logs: Array[String] = []
	...
	match effect.get("type", ""):
		"HEAL":
			...
			var healed: int = min(
				amount,
				new_player.get("max_hp", new_player.get("maxHp", 0)) - new_player.get("hp", 0)
			)
		...
		"APPLY_BUFF":
			var buff_id: String = str(effect.get("buffId", ""))
		...
		"SHIELD_EXPLOSION":
			var base_amount: int = effect.get("baseAmount", 0)
			var armor: int = new_player.get("armor", 0)
			new_player["armor"] = 0
			var total: int = base_amount + armor
			_apply_damage_to_all_skulls(new_board, total, logs, true)
		_:
			return {"success": false, "reason": "effect_not_supported"}

	var ability_level: int = player_ability.get(
		"current_level", player_ability.get("currentLevel", 1)
	)
	var cooldown: int = (
		ability_def.base_cooldown - (ability_def.cooldown_reduction_per_level * (ability_level - 1))
	)
...
func _apply_damage_to_all_skulls(
	board: Array, damage: int, logs: Array, consumes_armor := false
) -> void:
	if damage <= 0:
		return
	var defeated: int = 0
